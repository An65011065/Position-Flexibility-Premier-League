...
```{r}
pl_data<- read_csv("/Users/anweshanadhikari/Desktop/SLFinal/processed_data.csv")
...
conf_matrix <- confusionMatrix(data = factor(test_pred), 
                             reference = factor(test_data$main_position))
...
c<-confusionMatrix(data = as.factor(soccer_data_pred1$predicted_position), reference = as.factor(soccer_data_pred1$main_position))
```
## Ranger
```{r}
set.seed(12142024)
...
c<-confusionMatrix(data = as.factor(soccer_data_pred2$predicted_position), reference = as.factor(soccer_data_pred2$main_position))
```
## K-nearest neighbor
```{r}
set.seed(12142024)
...
c<-confusionMatrix(data = as.factor(soccer_data_pred3$predicted_position), reference = as.factor(soccer_data_pred3$main_position))
```
...
c<-confusionMatrix(data = as.factor(soccer_data_pred4$predicted_position), reference = as.factor(soccer_data_pred4$main_position))
```
...
c<-confusionMatrix(data = as.factor(soccer_data_pred5$predicted_position), reference = as.factor(soccer_data_pred5$main_position))
```
